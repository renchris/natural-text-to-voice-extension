...
- **Fast.** On an M1 Max a sentence is ready in about 0.4 seconds and a 400-word page in 6.5 to 8 seconds.
...
Chrome reads it with a macOS voice instead, through `chrome.tts`.
...
On an M1 Max the helper makes speech 22 to 26 times faster than real time. The wait you notice comes from long
selections, because playback starts only when all of the audio is ready.
...
- **No streaming yet.** The helper sends the WAV once all of it exists, so the wait above is also the time to first
  audio: under half a second for a sentence, 12 to 15 seconds for 5,000 characters, the longest selection it
  accepts.
