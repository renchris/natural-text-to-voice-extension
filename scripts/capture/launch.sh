...
#
# Usage: [HEADLESS=1] [HOLD_HEALTH=<port>:<ms>] [CDP_PORT=9555] [BLOCK=<ports>] [HOLD=1] scripts/capture/launch.sh <out-dir> [helper-port=8250] [site-dir]
#
...
#      light browser colour scheme, 1280x800 window at (40,60), CDP on CDP_PORT (default 9555).
...
#
#   CDP_PORT picks the remote-debugging port (default 9555), so two capture browsers can run side by side.
#   HOLD=1 keeps this script in the foreground until Chrome exits, then stops the guard: run it as a tracked
#   background job instead of leaving the browser orphaned.
#
...
if lsof -nP -iTCP:"$CDP_PORT" -sTCP:LISTEN >/dev/null 2>&1; then echo "port $CDP_PORT is in use: close the other capture browser first" >&2; exit 2; fi
...
nohup "$CFT" --user-data-dir="$PROFILE" --load-extension="$DIST" --test-type=gpu --use-mock-keychain \
  ...
  --window-size=1280,800 "${MODE_ARGS[@]}" --remote-debugging-port="$CDP_PORT" about:blank \
  </dev/null >"$OUT/chrome.log" 2>&1 &
CHROME_PID=$!
for _ in $(seq 1 50); do curl -s "127.0.0.1:$CDP_PORT/json/version" >/dev/null 2>&1 && break; sleep 0.2; done
WS=$(curl -s "127.0.0.1:$CDP_PORT/json/version" | node -pe 'JSON.parse(require("fs").readFileSync(0)).webSocketDebuggerUrl')
...
cat "$OUT/env.txt"
if [ "${HOLD:-0}" = 1 ]; then wait "$CHROME_PID" || true; kill "$GUARD_PID" 2>/dev/null || true; fi
