...
```bash
scripts/release/release.sh --version 1.5.0 --confirm v1.5.0 --confirm release/v1.5.0 --confirm renchris/homebrew-tap --confirm private-vulnerability-reporting
```
...
| **1. Tag** | Annotated tag `vX.Y.Z` at `origin/main`, pushed | `--confirm vX.Y.Z` | On origin at `HEAD` → `CURRENT`. On origin at another commit → **FAIL**: tags are never moved; bump the version |
